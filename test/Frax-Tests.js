...
contract('FRAX', async (accounts) => {
	...
	it('Check up on the oracles and make sure the prices are set', async () => {
		...
		// Handle FXS / WETH
		// Targeting 1620 FXS / 1 WETH
		await swapToPriceInstance.swapToPrice(
			fxsInstance.address,
			wethInstance.address,
			new BigNumber(1620e6),
			new BigNumber(1e6),
			new BigNumber(100e18),
			new BigNumber(100e18),
			COLLATERAL_FRAX_AND_FXS_OWNER,
	...
	it('Mint some FRAX using FXS (collateral ratio = 0) [mintAlgorithmicFRAX]', async () => {
		...
		// Mint some FRAX
		await pool_instance_USDC.mintAlgorithmicFRAX(fxs_amount, { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		...
		console.log("fxs_after change: ", fxs_after.toNumber() - bal_fxs);
		console.log("frax_after change: ", frax_after.toNumber() - bal_frax);
	...
	it('Redeem some FRAX for FXS (collateral ratio = 0) [redeemAlgorithmicFRAX]', async () => {
		...
		const fxs_before = new BigNumber(await fxsInstance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER)).div(BIG18);
		const frax_before = new BigNumber(await fraxInstance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER)).div(BIG18);
		console.log("fxs_before: ", fxs_before.toNumber());
		console.log("frax_before: ", frax_before.toNumber());
		...
		// Redeem some FRAX
		await pool_instance_USDC.redeemAlgorithmicFRAX(frax_amount, { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		// Collect redemption
		await time.advanceBlock();
	...
	it('Mint some FRAX using USDC as collateral (collateral ratio = 1) [mint1t1FRAX]', async () => {
		...
		// Mint some FRAX
		await pool_instance_USDC.mint1t1FRAX(collateral_amount, { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		...
		console.log("frax change: ", frax_after.toNumber() - bal_frax);
		console.log("collateral change: ", collateral_after.toNumber() - col_bal_usdc);
		console.log("pool collateral change: ", pool_collateral_after.toNumber() - pool_bal_usdc);
	...
	it('Have the pool buy back FXS with excess USDC (if available) [buyBackFXS]', async () => {
		...
		console.log("fxs_price: $", (1 / fxs_price.toNumber()).toFixed(6));

		// Buy back some FRAX
		await pool_instance_USDC.buyBackFXS(fxs_amount, { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		...
		console.log("col_bal_usdc change: ", collateral_after.toNumber() - col_bal_usdc);
		console.log("pool_bal_usdc change: ", pool_collateral_after.toNumber() - pool_bal_usdc);
		console.log("global_collateral_value change: ", global_pool_collateral_after.toNumber() - global_collateral_value);
	...
	it('Mint some FRAX using FXS and USDC (collateral ratio between .000001 and .999999) [mintFractionalFRAX]', async () => {
		...
		await pool_instance_USDC.mintFractionalFRAX(collateral_amount, fxs_amount, { from: COLLATERAL_FRAX_AND_FXS_OWNER });
	...
	it('Redeem some FRAX for USDC (collateral ratio >= 1) [redeem1t1FRAX]', async () => {
		...
		// Redeem some FRAX
		await pool_instance_USDC.redeem1t1FRAX(frax_amount, { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		// Collect redemption
		await time.advanceBlock();
		await pool_instance_USDC.collectRedemption({ from: COLLATERAL_FRAX_AND_FXS_OWNER });
	...
	it('Redeem some FRAX for FXS and USDC (collateral ratio between .000001 and .999999) [redeemFractionalFRAX]', async () => {
		...
		// Mint some FRAX to move the ratio below 1
		const collateral_amount = new BigNumber("2500000e18");
		await col_instance_USDC.approve(pool_instance_USDC.address, collateral_amount, { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		await pool_instance_USDC.mint1t1FRAX(collateral_amount, { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		...
		col_bal_usdc = collateral_before;
		pool_bal_usdc = pool_collateral_before;
		console.log("bal_fxs: ", bal_frax.toNumber());
		console.log("bal_frax: ", bal_frax.toNumber());
		console.log("col_bal_usdc: ", col_bal_usdc.toNumber());
		console.log("pool_bal_usdc: ", pool_bal_usdc.toNumber());
	...
	it('recollateralizes the protocol using [recollateralizeFrax]', async () => {
		...
		const pre_frax_supply = new BigNumber(await fraxInstance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER)).div(BIG18);
		...
		const pre_global_collat_value = new BigNumber(await fraxInstance.globalCollateralValue.call()).div(BIG18);
		console.log("global_collat_value", pre_global_collat_value.toNumber());

		const frax_price = new BigNumber(await fraxInstance.frax_price.call()).div(BIG6);
		console.log("frax price:", frax_price.toNumber());

		//await col_instance_USDC.approve(col_instance_USDC.address, THREE_THOUSAND_DEC18, { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		//await col_instance_USDC.transferFrom(pool_instance_USDC.address, col_instance_USDC.address, THREE_THOUSAND_DEC18, { from: COLLATERAL_FRAX_AND_FXS_OWNER });

		await fraxInstance.mint(new BigNumber(500000e18), { from: COLLATERAL_FRAX_AND_FXS_OWNER });
		...
		const frax_supply = new BigNumber(await fraxInstance.balanceOf.call(COLLATERAL_FRAX_AND_FXS_OWNER)).div(BIG18);
		...
		const global_collat_value = new BigNumber(await fraxInstance.globalCollateralValue.call()).div(BIG18);
